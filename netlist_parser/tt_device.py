...
class Device(TTObject):
    ...
    def create(arch, device_id, cluster_desc, device_desc_path: str, context: None):
        dev = None
    ...
    def _create_nocVirt_to_nocTr_map(self):
        # WZ: No harvesting nodes for now
    ...
    def __init__(self, id, arch, cluster_desc, address_maps: Dict[str, AddressMap], device_desc_path: str, context: None):
        """

        Args:
            address_maps (Dict[str, AddressMap]): map of core_type(str) -> AddressMap
            WZ: We assume every device has MMIO, no harvesting nodes for now
            We do not have devices, so that we shield the server queries
        """
        self._id = id
        self._arch = arch
        ...
        harvesting_desc = cluster_desc.get("harvesting", None)
